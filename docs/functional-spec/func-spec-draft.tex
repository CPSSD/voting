...
Each voting system would require its own rules to specified for the vote to be successful. For this reason the \proj{} system will focus on only one or two systems as a proof of concept, with the ability to add more systems being kept in mind during development. The two candidate systems will be \textit{Proportional Representation with a Single Transferable Vote}\footnote{The PR-STV system is used in Ireland.} and \textit{Party List Proportional Representation}\footnote{The Party List PR system is used in 85 countries.}.
