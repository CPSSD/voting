...
\begin{description}
    ...
    \item[Peer-to-peer network \texttt{(P2P)}]
        A peer-to-peer network is a distributed networking architecture in which peer nodes are equal and can act as both a "client" and a "server" for other nodes on the network. It eliminates the need for a centralized server. We will refer to the peer-to-peer network as a \texttt{P2P} network from here on.
...
The \proj{} system is designed to enable electronic voting to tamper-proof, anonymous and to be carried out securely. A user of the system will cast a \textit{ballot} electronically through some user interface. The user's \textit{ballot} is then encrypted to form a secure \textit{transaction}. The user can verify that their \textit{ballot} is as they intended by decrypting it. This will spoil and invalidate the \textit{ballot}, but after repeated casting and decrypting the user's certainty in the system can be assured\footnote{A user can never be 100\% certain that their vote is as intended without spoiling their ballot. The probability that their ballot has been tampered with is denoted by \[P(t) = (1/2)^n\] where \[t\] is the event in which a ballot has been tampered with and \[n\] is the number of times a user decrypts their vote to check it. After only 7 encryptions and decryptions, the user can be sure with a greater than 99\% probability that their ballot has not been tampered with.}. When the user is happy with their \textit{ballot}, the \textit{transaction} is then broadcast to all of the client's peers to be verified. The veriInfication of the \textit{transaction} requires a proof of work to be completed by the peers. This proof of work entails some computationally expensive task. When the proof of work is complete, the \textit{block} is then broadcast to a client's peers. If there is disagreement between a new \textit{block}, consensus is used to select the correct version which is then broadcast to all peers.
To verify their vote, a user can look at the blockchain and verify that their receipt exists on the chain. Homomorphic encryption\footnote{Homomorphic encryption allows computations to be carried out on ciphertext, without the need to reveal the value of the plaintext. This creates an encrypted result which, when decrypted, matches the result of the same operations being performed on the plaintext.} of votes allows them to be tallied without decrypting individual votes. Any user should be able to perform the computation to tally the votes.

\subsection{User Characteristics}
A user will be any eligible voter. The users are not expected to require any prior knowledge of \textit{blockchains}, cryptography or other technical concepts described in this document. Because different users may have different user needs or certain levels of ability, the system should be accessible. As a graphical user interface is not the main focus of the system, this challenge will not be addressed to any major extent. The system will be used via text inputs, similar to a command line program, on top of which a UI could be built at a later date.

A user should be able to input a \textit{vote}, and verify their selection. Once the user has made their selections, they should be able to validate the encryption of their \textit{ballot} as many times as they see sufficient. Once the user is happy that the system is registering their intended selections, the user should then be able to cast their \textit{ballot} as a \textit{transaction} to the network to have it verified and added to the \textit{ledger}. The user should also receive a digital receipt of their \textit{transaction} so that they can verify their \textit{ballot} exists at a later date. The user should also be able to perform the required computation to tally the votes should they wish to do so.

\subsection{Operational Scenarios}
\subsubsection{Main}
The main scenario in which the system is intended to be used is any large scale elections, referendums or other votes in which a large body of people will participate. These scenarios are required to provide anonymity to voters, tamper-proof security, easy tallying and re-counting of ballots, and a trustless\footnote{A trustless environment is one in which no one individual or group requires to be trusted to ensure the integrity of the system. In a paper ballot, you don't trust any one person to be alone with a ballot box during transport. However, in a paper ballot, people can be threatened, bribed or incabable of securing a ballot box from being tampered with.} environment.
...
\begin{enumerate}
    \item
        A voter should not be able to sell their vote. For this constraint to be met, a user should have no way of proving what way they filled out their ballot to the outside world. This also means that the ballot itself should have no way of identifying who filled it out.
...
Each voting system would require its own rules to specified for the vote to be successful. For this reason the \proj{} system will focus on only one or two systems as a proof of concept, with the ability to add more systems being kept in mind during development. The two candidate systems will be \textit{Proportional Reprisentation with a Single Transferable Vote}\footnote{The PR-STV system is used in Ireland.} and \textit{Party List Proportional Representation}\footnote{The Party List PR system is used in 85 countries.}.
